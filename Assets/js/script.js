...
// If prompts filled out correctly, password will be generated 
  if (correctPrompts) {
    var customizedPassword = generatePassword();
    var passwordText = document.querySelector("#password");
    passwordText.value = customizedPassword;
  } else {
    passwordText.value = "";
  }
}
...
// Randomizes contents of password
function generatePassword() {
  var password = "";
  for (var i = 0; i < charLength; i++) {
    var randomIndex = Math.floor(Math.random() * chArr.length);
    password = password + chArr[randomIndex];
  }
  return password;
}
